...
- [x] 마이그레이션 없음(kind "place" 는 새 VALUE — DDL 불필요, Moim.location/PollOption.optionDate 이미 존재, PK/FK/인덱스 무변경, 스키마 파일 무변경), prisma migrate status 변경 없이 clean(13 마이그레이션 — MOIM-010 추가 없음). ✓ migrate status clean 2026-06-22 (AC-1)
- [x] `POST /moims/:id/polls` 가 kind="place" + 자유 텍스트 장소 옵션 수용(normalizeOptions label 저장, optionDate null — 일반과 동일 경로) + 미지 kind 400 + 일반/날짜 투표 무변경 + question 빈/옵션<2 400 + 비멤버 403. ✓ backend jest 308/308 (AC-2)
- [x] `POST .../close` 가 장소 투표 단일 승자 → Moim.location 확정(finalizedLocation) / 동점 → "tie" 스킵 / 무표 → "no_votes" 스킵 / 일반 투표 → location finalize 안 함 / 날짜 투표 → startsAt 확정(MOIM-008 회귀, finalizedLocation null) + 기존 location 덮어쓰기 + 비생성자 403(finalize 미실행). ✓ poll-place-finalize.live.mts 13/13 PASS 2026-06-22 (AC-3)
- [x] `Moim.location` 쓰기가 `MoimService.setLocation` 단일 메서드를 통한다(closePoll 호출, createMoim 외 유일 쓰기 경로, setStartsAt 미러). ✓ backend jest + tsc 0 (AC-3)
- [x] `GET /moims/:id/polls` 가 각 poll 의 kind(="place" 포함) + 각 옵션 label/optionDate(장소 투표 null) + 마감/finalize 된 장소 투표 결과 조회 가능 + 비멤버 403 + poll 없으면 빈 배열. ✓ backend jest 308/308 (AC-4)
- [x] close 응답이 finalizedLocation(string|null) + finalizedStartsAt 과 상호 배타 + finalizeSkippedReason + vote/list 응답은 셋 다 null. ✓ poll-place-finalize.live.mts PASS (AC-5)
