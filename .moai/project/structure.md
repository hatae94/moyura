...
│  │  │  ├─ push/       # FCM 푸시 도메인 모듈 (SPEC-CHAT-002) — PushModule/PushListener(@OnEvent 단방향, chat↛push 의존 방향 없음) + FcmSender(firebase-admin, graceful no-op) + DeviceTokenService(upsert/unregisterByOwner owner-scoped) + DeviceTokenController(POST /devices, DELETE /devices/:token) + dto(register-device/device-token-response) + *.spec.ts + loose-coupling.spec.ts. chat 모듈은 push 존재 미인지 — push는 chat-events.ts(@MX:ANCHOR) 계약에만 단방향 의존. **알림 탭 핸들러(R-PUSH-007)는 모바일 측 `app/_layout.tsx`/`(tabs)/home/[id].tsx` 에 배선됨(2026-06-25, v0.4.0) — 백엔드 발송 경로 무변경.**
│  │  │  ├─ safety/      # 신고·차단 도메인 모듈 (SPEC-SAFETY-001) — SafetyModule/SafetyService/SafetyController: POST /reports(신고 생성), POST/DELETE/GET /blocks(차단 CRUD). getHiddenUserIds(sub) 뷰어별 숨김 필터(block 정방향 + report 신고항). Block/Report 테이블(마이그레이션 20260702100000_add_safety, RLS default-deny). 커밋 150ea8e
│  │  │  ├─ account/     # 회원 탈퇴 도메인 모듈 (SPEC-ACCOUNT-001) — AccountModule/AccountService/AccountController: DELETE /me/account(탈퇴 오케스트레이션 — profile 삭제, WithdrawnAccount 툼스톤 생성, moim_member.withdrawnAt 마킹, SUPABASE_SERVICE_ROLE_KEY로 auth 사용자 삭제). 마이그레이션 20260702200000_add_withdrawn_account. 커밋 88432e7
│  │  │  ├─ expense/     # 모임 회비/정산 도메인 모듈 (SPEC-MOIM-EXPENSE-001) — ExpenseModule/ExpenseService/ExpenseController(@Controller('moims/:id/expenses'): POST 기록/GET 목록+집계/PATCH :expenseId 수정/DELETE :expenseId 삭제) + SettlementController(@Controller('moims/:id/settlements'): POST 정산 완료 마킹/DELETE 전체·DELETE :settlementId 해제) + dto(create-expense/update-expense/expense-response) + *.spec.ts + integration.spec.ts. MoimModule import(assertMember 재사용). Expense(amount Int/category/payerUserId/memo?) + ExpenseShare(복합 PK (expenseId,userId), shareAmount) + Settlement(fromUserId/toUserId/amount) 3 테이블. realtime broadcast 트리거(20260624110000_add_expense_realtime_broadcast — CHAT-001 `moim:{id}` 채널 재사용).
│  │  │  ├─ prisma/     # PrismaService (pg adapter, pingDatabase)
│  │  │  └─ generated/  # Prisma 7 source-emit 클라이언트 (gitignore, 재생성)
│  │  ├─ prisma/        # schema.prisma (Profile(name? — SPEC-MOBILE-004) + Moim(startsAt?/location? — SPEC-MOIM-004, polls Poll[] 역참조 — SPEC-MOIM-005) + MoimMember + MoimInvite + ChatMessage + DeviceToken + Poll(multiSelect Boolean @default(false) — SPEC-MOIM-006 / closesAt DateTime? — SPEC-MOIM-007 / kind String @default("general") — SPEC-MOIM-008)/PollOption(optionDate DateTime? — SPEC-MOIM-008)/PollVote(복합 PK (pollId,optionId,userId) — SPEC-MOIM-006) 모델) + migrations/20260602095934_init_profile + 20260613155202_add_moim + 20260613171209_add_moim_invite + 20260613175232_add_chat + 20260614_add_device_token + 20260615000000_add_profile_name(SPEC-MOBILE-004) + 20260619000000_add_moim_event_fields(SPEC-MOIM-004) + 20260619100000_add_poll(SPEC-MOIM-005 — Poll/PollOption/PollVote 신규 3 테이블) + add_poll_multi_select(SPEC-MOIM-006 — Poll.multiSelect 추가 + PollVote PK 비파괴 변경) + 20260620200000_add_poll_closes_at(SPEC-MOIM-007 — Poll.closesAt nullable additive 추가) + 20260621000000_add_poll_kind_option_date(SPEC-MOIM-008 — Poll.kind TEXT NOT NULL DEFAULT 'general' + PollOption.option_date nullable additive 추가) + 20260622000000_add_poll_realtime_broadcast(SPEC-MOIM-009 — broadcast_poll_change 함수 + poll_broadcast/poll_vote_broadcast 트리거 additive 추가, 테이블/컬럼 무변경, 순수 트리거) + 20260624000000_add_moim_max_members_and_member_realtime(SPEC 멤버 cap+realtime — Moim.maxMembers Int @default(15) 추가 + 멤버 목록 broadcast 트리거) + 20260624100000_add_expense(SPEC-MOIM-EXPENSE-001 — Expense/ExpenseShare/Settlement 신규 3 테이블) + 20260624110000_add_expense_realtime_broadcast(expense broadcast 트리거 additive 추가, CHAT-001 채널 재사용). **Expense(amount Int/category/payerUserId/memo?/createdBy) + ExpenseShare(복합 PK (expenseId,userId)/shareAmount) + Settlement(fromUserId/toUserId/amount/settledBy) 모델 추가 — SPEC-MOIM-EXPENSE-001. Moim.maxMembers Int @default(15) 추가.**
...
│  │  └─ eas.json       # EAS local/local-sim/production/production-local 프로파일(projectId 링크 + expo-dev-client; production-local=extends production + 로컬 google-services 경로 — eas build --local 은 클라우드 secret env 미적용이라 로컬 파일 주입, 2026-06-25)
│  └─ web/              # @moyura/web     — Next.js 16 (app/, public/)
│     ├─ .storybook/    # Storybook react-vite 배선(SPEC-WEB-STORYBOOK-001) — main.ts(@storybook/react-vite 빌더, addons:[] 코어-only, viteFinal로 @/ alias+React 플러그인) + preview.tsx(../app/globals.css import — 실 디자인 시스템 렌더). 빌드 타깃 분리(web 런타임 무영향)
│     ├─ components/ui/ # 공용 UI primitive 카탈로그(SPEC-WEB-STORYBOOK-001) — Button/Card/Avatar/Input/Badge + cn.ts 헬퍼, 각 colocate *.stories.tsx(CSF3). presentational-only(next/*·supabase·bridge·네트워크 import 0), globals.css 시맨틱 토큰을 Tailwind 유틸로 소비(신규 토큰 0)
...
│     │  ├─ (main)/     # 탭 라우트 그룹 (SPEC-MOBILE-003) — layout.tsx(BottomTabBar·인증가드·ShellSessionAnnouncer·ShellModeEffect·require-named-session 가드 — SPEC-MOBILE-004) + _components/(BottomTabBar·PlaceholderTab·ShellModeEffect·ShellSessionAnnouncer) + home/(page·HomeTab·[id]/page + loading.tsx 라우트별 스켈레톤 — SPEC sin1/transition-perf 2026-06-25) + explore/notifications(플레이스홀더) + profile/(마이 — page.tsx Server Component[requireNamedSession → 이메일/가입일 read-only + 표시 이름] + profile-form.tsx[이름 수정 Client 폼] + actions.ts[updateProfileAction → patchMe] + 로그아웃 — SPEC-PROFILE-001). **레이아웃/스크롤 모델(2026-06-25, 구현·배포됨·실기기 검증 대기)**: 셸은 document-scroll(normal-flow) + BottomTabBar `fixed inset-x-0 bottom-0 z-40`(+safe-area), 콘텐츠 `pb-bottom-tab` 클리어런스, 헤더 `sticky top-0` — 모바일 브라우저 크롬 접힘 + 탭 비잘림 동시(고정 svh 앱셸 접근 supersede). 채팅(moims/[id]/chat)은 예외(`h-dvh-fixed` 내부 스크롤). globals.css `@utility pt-page`·`h-dvh-fixed`·`pb-bottom-tab`, 루트 layout.tsx `viewport-fit=cover`
...
│     │     └─ [id]/expenses/  # 모임 회비/정산 페이지 (SPEC-MOIM-EXPENSE-001) — page.tsx(Server Component, GET /moims/:id/expenses 집계 조회) + expenses-view.tsx(Client — 기록 목록·카테고리·정산 시각화) + expense-actions.ts(Server Action — 기록/수정/삭제/정산 마킹 + revalidatePath). 헬퍼: lib/moim/expenses.ts
...
├─ .github/workflows/   # ci.yml (install/build/lint/test/typecheck, deploy 없음) + migrate-prod.yml (master 푸시 시 apps/backend/prisma/migrations/** 변경 → prisma migrate deploy 자동 적용, PROD_DIRECT_URL 시크릿, workflow_dispatch 지원 — 2026-06-25)
...
| `@moyura/web` | `apps/web` | 메인 UI 표면 (App Router) | Next.js `16.2.6`, react `19.2.4`, Tailwind v4, TypeScript `^5`, Storybook `10.5.3`(react-vite) | 스캐폴드 |
