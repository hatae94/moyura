...
> 네이티브 Google 로그인 + provider-agnostic 이름 온보딩은 [`SPEC-MOBILE-004`](../specs/SPEC-MOBILE-004/spec.md)(status: `in-progress`, v0.2.0)에서 **자동 게이트 통과**(브랜치 `feature/SPEC-MOBILE-004`, 커밋 `80d565e`, evaluator-active Overall PASS Func 75/Sec 75/Craft 75/Consistency 90). backend jest 214/214(85.36% branch), mobile vitest 187/187, tsc 0 errors, web build OK, expo export OK, prisma migrate status clean. Profile.name(nullable) 마이그레이션, PATCH /me, provider-agnostic 온보딩 가드(`require-named-session.ts`), mobile 네이티브 Google Sign-In 코어(google-signin-core/signin-id-token-core 순수 vitest 코어 + SDK 래퍼). device-gated AC(1/2/3/5/6a/6b)는 EAS dev build + 실 Google 계정 필요 → in-progress 유지.
...
> 모임 채팅 코어는 [`SPEC-CHAT-001`](../specs/SPEC-CHAT-001/spec.md)(status: `completed`, v0.3.0)에서 **라이브 E2E 검증 완료**(브랜치 `feature/SPEC-MOBILE-004`, 커밋 `f3fe178`, evaluator PASS). ChatMessage 모델 + 트리거/RLS 마이그레이션, sendMessage/getHistory, chat.message.created 이벤트 계약, 웹 채팅 UI 구현. 2026-06-15 로컬 Supabase 라이브 E2E로 AC-1c(멤버 broadcast 수신) / AC-4(비멤버 RLS 구독 거부) / AC-5(CSP wss 연결) 전부 PASS → completed 전환. 신규 의존성: `@nestjs/event-emitter@^3.1.0`. Supabase Realtime broadcast_changes + trigger/RLS 인프라 도입.
...
| **IMPLEMENTED (SPEC-CHAT-001, completed — 라이브 E2E 검증 2026-06-15 PASS)** | 모임 채팅 코어: ChatMessage 모델(BigInt PK, moimId FK→moim Cascade, @@index(moimId,id desc)) + 마이그레이션 `20260613175232_add_chat`(content CHECK 1..2000, RLS default-deny, broadcast_chat_message() SECURITY DEFINER, chat_message_broadcast 트리거, realtime.messages SELECT 정책). sendMessage(assertMember + insert + best-effort emit) / getHistory(keyset 내림차순). chat.message.created 이벤트 계약(@MX:ANCHOR, chat-events.ts). 웹 채팅 UI(useChatChannel + /moims/[id]/chat/page.tsx, nickname 클라이언트 해석). CSP wss 호스트 고정. jest 170/170(chat 22), chat 100% stmt/85.71% branch, evaluator PASS(Func 90/Sec 82/Craft 85/Consistency 90). 라이브 E2E: AC-1c(멤버 broadcast 수신 PASS) / AC-4(비멤버 RLS 구독 거부 PASS) / AC-5(CSP wss 연결 PASS). 신규 의존성: `@nestjs/event-emitter@^3.1.0`. |
...
- **FCM 푸시 인프라**(SPEC-CHAT-002): `firebase-admin@^13.10.0`(backend, Node 20+ 호환) + `expo-notifications@~56.0.17`(mobile, SDK 56). `FIREBASE_CREDENTIALS` env(optional Zod, JSON 직렬화 서비스 계정 키) — `apps/backend/src/config/env.validation.ts`에 선언. 부재 시 FcmSender no-op(개발/CI 환경에서 부팅·테스트 차단 없음). 실 FCM 발송은 Firebase 프로젝트 셋업 + dev build 필요(device-gated).
