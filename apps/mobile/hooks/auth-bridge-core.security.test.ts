...
describe("decideWebViewLoad (R-T9 / AC-T9: WebView origin 잠금 + OAuth 인터셉트 보존 — C-2)", () => {
  ...
  it("신뢰 WEB_URL origin 의 top-level 로드는 trusted-load (in-WebView 허용)", () => {
    expect(decideWebViewLoad("http://localhost:3000/", ctx)).toBe("trusted-load");
    expect(decideWebViewLoad("http://localhost:3000/me", ctx)).toBe("trusted-load");
    expect(decideWebViewLoad("http://localhost:3000/login?error=x", ctx)).toBe(
      "trusted-load",
    );
  });
  ...
  it("신뢰 호스트의 다른 포트/scheme 는 deny (origin exact — 서브도메인/포트/scheme 불일치)", () => {
    expect(decideWebViewLoad("http://localhost:4000/me", ctx)).toBe("deny");
    expect(decideWebViewLoad("https://localhost:3000/me", ctx)).toBe("deny");
    expect(decideWebViewLoad("http://sub.localhost:3000/me", ctx)).toBe("deny");
  });
...
describe("buildTargetOrigin (R-T8 / AC-T8: specific targetOrigin — NOT '*' — C-2(d)/H-1)", () => {
  it("신뢰 WEB_URL 의 origin literal 을 반환한다 (와일드카드 아님)", () => {
    expect(buildTargetOrigin("http://localhost:3000")).toBe("http://localhost:3000");
    expect(buildTargetOrigin("http://localhost:3000/me")).toBe("http://localhost:3000");
    expect(buildTargetOrigin("http://10.0.2.2:3000/")).toBe("http://10.0.2.2:3000");
  });

  it("절대 '*' 를 반환하지 않는다 (토큰 브로드캐스트 차단)", () => {
    expect(buildTargetOrigin("http://localhost:3000")).not.toBe("*");
  });
});
