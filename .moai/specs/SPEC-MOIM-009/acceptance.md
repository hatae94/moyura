...
- [x] `poll`(AFTER INSERT OR UPDATE) + `poll_vote`(AFTER INSERT OR DELETE) broadcast 트리거 함수(`broadcast_poll_change` — `SECURITY DEFINER`/`search_path=''`/이벤트 `'poll_change'`/토픽 `'moim:'||moimId`/페이로드 `{moimId,pollId}`) additive 추가 + 테이블/컬럼/PK/FK/인덱스 무변경(기존 row 보존, 손실 0), prisma migrate clean(hand-authored 비파괴 `20260622000000_add_poll_realtime_broadcast`). ✓ 라이브 검증 2026-06-22 (AC-1)
- [x] 채널 `moim:{id}` + `realtime.messages` 멤버십 RLS 재사용(새 채널/새 RLS 0), 멤버 A·B 수신 + 비멤버 RLS 차단(미수신), 구별 이벤트명 `'poll_change'`로 채팅/투표 교차 수신 0. ✓ poll-realtime.live.mts 7/7 PASS 2026-06-22 (AC-2)
- [x] 백엔드 NestJS 코드 무변경(broadcast 순수 DB 트리거 — 기존 create/vote/close가 row 변경으로 AFTER ROW 발화), poll 도메인 응답/DTO/인가 보존, 백그라운드 잡/크론/큐 없음. ✓ backend jest 301/301(회귀 0) (AC-3)
- [x] PollsSection이 accessToken으로 `moim:{id}` 구독(setAuth + private + `'poll_change'`) + 수신 시 router.refresh(Server Component 재실행 → 집계 + 자신의 myVotes + 헤더 startsAt) + 토큰 가드 + 언마운트 removeChannel(useChatChannel 미러). ✓ web tsc/lint/build 0 — PENDING 브라우저 다중 클라이언트 라이브 워크스루 (AC-4)
- [x] page.tsx가 `accessToken={session.access_token}` 전달 + 기존 props(moimId/polls/currentUserId)·fetch·가드 보존 + poll fetch/render 로직 무변경. ✓ web tsc/lint/build 0 (AC-5)
- [x] 모바일 무변경(WebView가 구독하는 웹 클라이언트 실행 — 신규 네이티브 0), mobile tsc/vitest/expo export 회귀 0. ✓ mobile vitest 215/215(회귀 0) — PENDING iOS WebView 실시간 검증 (AC-6)
