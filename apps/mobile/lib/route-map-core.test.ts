...
// routeForUrl/isCrossRoute 모두에서 비-디스패치(null/false)여야 한다. 만약 누군가 invite 를 앱 라우트로 승격하면
// in-WebView 히스토리 모델이 깨져 router.back() 이 더는 탐색으로 복귀하지 않는다 — 이 블록이 그 회귀를 RED 로
// 잡는다. (웹 측 런타임 RED→GREEN 은 apps/web 에 테스트 하니스가 없어 도달 불가 — 이 순수 불변식 잠금 +
// 시뮬레이터 수동 재현으로 대체한다.)
