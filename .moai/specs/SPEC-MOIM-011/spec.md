...
이는 **초대 생성 버튼(owner 전용) + 발급 헬퍼 + 링크 표시·복사 + 모바일 딥링크 라우트(WebView 호스팅) + 웹 수락 페이지 "앱에서 열기" 버튼** 이지 대형 기능이 아니다. Universal Links·자동 scheme 리다이렉트·초대 목록/폐기/관리 UI·per-invite 만료/maxUses 입력·QR·OS 공유 시트·Android 딥링크·백엔드 변경은 모두 제외한다(§4).
...
- (Event-driven, scheme 발화) **WHEN** 사용자가 "앱에서 열기" 를 누르면, **the web app shall** `window.location` 을 `moyura://invite/{token}` 으로 설정해 커스텀 scheme 을 발화한다(현재 path 의 token 을 그대로 사용 — 새 노출 없음).
- (State-driven, 폴백) **WHILE** 앱이 설치되어 있지 않아 scheme 이 no-op 인 동안(또는 사용자가 앱 대신 웹을 선호하는 동안), **the web app shall** 기존 닉네임 수락 폼을 웹 폴백으로 유지한다 — **자동 리다이렉트를 하지 않는다**(scheme 은 버튼 클릭으로만 발화, 사용자가 웹에서 그대로 가입 가능).
...
### 웹→앱 핸드오프 = "앱에서 열기" 버튼 (모바일 한정, 자동 아님)

- 웹 수락 페이지(`/invite/[token]/page.tsx`, Client Component)에 모바일 브라우저 감지 시 "앱에서 열기" 버튼을 추가한다 — 클릭하면 `window.location = moyura://invite/{token}`(현재 path 의 token). 앱 설치 시 OS 가 앱을 열고(위 딥링크 라우트), 미설치 시 scheme 은 no-op 이라 사용자는 그대로 웹 닉네임 폼으로 가입한다.
- **자동 리다이렉트 안 함**: 페이지 로드 시 scheme 으로 자동 점프하면 앱 미설치 사용자가 "이 페이지를 열 수 없음" 류 오류를 보거나 멈춘다 — 명시적 버튼 클릭이 가장 안전한 MVP 기본값이다(§4). 데스크톱은 버튼 미노출(scheme 이 무의미).
...
| 자동 scheme 리다이렉트로 미설치 사용자 멈춤 | MEDIUM | 자동 점프 시 앱 미설치 사용자가 오류/멈춤. **버튼 클릭으로만** scheme 발화(자동 X) + 웹 폴백 항상 유지로 회피(확정 결정). 데스크톱 버튼 미노출. |
