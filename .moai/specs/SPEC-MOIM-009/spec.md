...
- 2026-06-23 (v0.3.0): **device-gated 검증 완료 → status completed**. 모바일 iOS WebView cross-surface 실시간 갱신을 직접 관찰: 시뮬레이터 앱(로그인 멤버)이 "웹 검증 모임" 상세 WebView를 연 상태에서 **데스크톱(앨리스, 방장)이 "테스트 투표" 옵션에 투표**하자 — 앱을 전혀 건드리지 않았는데 — 앱 WebView의 해당 투표가 **새로고침 없이** 0표→1표·100%(총 0표→1표)로 live 갱신됨(스크린샷 확인). `poll_vote` 변경 → `broadcast_poll_change()` → `'poll_change'` on `moim:{id}` → 앱 WebView의 `usePollChannel` → `router.refresh` 체인이 iOS 앱 WebView 안에서 동작 실증. 추가로 **데스크톱 멀티탭**(같은 세션 2탭) 검증: 한 탭에서 "점심 메뉴는?" 김밥 투표 → 다른 탭이 새로고침 없이 총 1표→2표(김밥 0→1표)로 갱신 확인. 기존 백엔드/Realtime E2E(poll-realtime.live.mts 7/7, 2026-06-22) + CHAT-001 동일 `moim:{id}` 채널의 WebView 동작과 합치 — device-gated AC-7 충족. 검증 도구: chrome-devtools(데스크톱 멀티탭) + 시뮬레이터(iPhone 16) simctl/screenshot(앱 in-WebView 관찰). 참고: 시뮬레이터 앱 로그인은 사용자가 직접 수행(Google) 후 진행.
- 2026-06-22 (v0.2.0): 구현 완료 + 백엔드/Realtime 라이브 E2E 검증(poll-realtime.live.mts 7/7 PASS) → status in-progress(device-gated). **구현 요약**: (1) **마이그레이션 `20260622000000_add_poll_realtime_broadcast`(비파괴 순수 트리거)** — plpgsql 함수 `broadcast_poll_change()`(`SECURITY DEFINER`, `SET search_path=''`)를 신규 정의. `AFTER INSERT OR UPDATE ON poll`(`poll_broadcast`) + `AFTER INSERT OR DELETE ON poll_vote`(`poll_vote_broadcast`) 트리거 2개 additive 추가. 테이블·컬럼·PK·FK·인덱스 무변경(row 손실 0). 함수는 `realtime.send(jsonb_build_object('moimId',...,'pollId',...), 'poll_change', 'moim:'||moimId, true)` — 경량 신호(집계/표 정보 미포함). `poll_vote` 트리거는 `poll_id` → `public.poll.moim_id` 조회로 moimId 해소(`poll_vote`에 moim_id 컬럼 없음). `poll` 트리거는 `NEW.moim_id` 직접 사용. `DROP TRIGGER IF EXISTS` 멱등 가드 포함. 이벤트명 `'poll_change'`(채팅 `'INSERT'`와 구별 — 교차 수신 방지). (2) **백엔드 NestJS 무변경** — broadcast는 순수 DB 트리거, 기존 `POST create/vote/close`가 이미 트리거를 발화시키는 row를 변경한다(NestJS 코드 0줄 변경). (3) **웹 실시간 구독**: `apps/web/lib/poll/usePollChannel.ts` 신규 훅(useChatChannel 미러 — `createClient()` → `realtime.setAuth(accessToken)` → `.channel('moim:'+id, {config:{private:true}}).on('broadcast',{event:'poll_change'},onChange).subscribe()` + 언마운트 `removeChannel`). `polls-section.tsx` `PollsSection`에 `accessToken: string | null` prop + `useRouter`/`useCallback`으로 usePollChannel 배선(수신 시 `router.refresh()`). `page.tsx`에 `accessToken={session.access_token}` prop 전달 추가. (4) **LIVE 종단 증명 스크립트** `apps/backend/test/poll-realtime.live.mts` 신규(chat.live.mts/poll-finalize.live.mts 패턴 미러). (5) **모바일 무변경** — 상세 `/home/[id]`는 in-WebView, WebView가 구독하는 웹 클라이언트를 이미 실행. **자동 게이트**: web tsc/lint/nx build 0, backend jest 301/301(코드 변경 없음 — 회귀 green; 별도 커밋 eda85a2에서 pre-existing 시간 의존 테스트 수정, 본 SPEC 범위 외), mobile vitest 215/215(회귀 0), prisma migrate status clean(12 마이그레이션). **Realtime 라이브 E2E(2026-06-22 — poll-realtime.live.mts, 실 Supabase 스택)**: 7/7 PASS — 멤버 2명이 한 멤버의 poll 생성(poll INSERT)/투표(poll_vote INSERT)/마감(poll UPDATE) 시 `'poll_change'` broadcast를 둘 다 수신(setAuth + private 채널 + 멤버십 RLS 통과) + 비멤버는 RLS 차단(미수신) + 시그널 페이로드가 경량 `{moimId, pollId}`(집계/표 정보 없음) 확인. **미완료 device-gated**: 브라우저 다중 클라이언트 UI 워크스루(탭 2개 — moyura-verify 세션 만료) + 모바일 iOS WebView 실시간 갱신 검증(SPEC-MOIM-005~008과 함께 배치). 자동 게이트 + LIVE 스크립트 단독으로 completed 전환 불가(프로젝트 메모리 규칙: mobile WebView SPEC device-gated).
