...
│     ├─ app/           # auth/callback/route.ts(PKCE 콜백), login/, me/(require-named-session 가드 적용 — SPEC-MOBILE-004), invite/[token]/(초대 랜딩 — 익명 로그인 → nickname → accept → /moims/[id]/chat; **"앱에서 열기" 버튼 추가 — useSyncExternalStore UA 감지로 모바일 브라우저 한정 노출, 클릭 시 window.location = moyura://invite/{token}, 자동 리다이렉트 없음, 기존 닉네임 폼 보존 — SPEC-MOIM-011**), onboarding/(이름 입력 온보딩 — SPEC-MOBILE-004, (main) 그룹 외부, 루프 안전)
