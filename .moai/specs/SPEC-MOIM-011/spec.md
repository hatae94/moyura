...
- 2026-06-22 (v0.5.0): **이미 가입된 회원 초대 시 기존 닉네임 prefill**(사용자 요청 — "이미 가입된 회원이 링크로 들어오면 기존 닉네임이 input 에 입력돼 있도록"). 수락 페이지를 Server Component(`app/invite/[token]/page.tsx`) + Client 폼(`app/invite/[token]/invite-accept-form.tsx`)으로 분리 — 서버가 쿠키 세션을 선택적으로 읽어 로그인 회원이면 `GET /me` 의 `Profile.name` 을 `initialNickname` prop 으로 내려 닉네임 input 초기값으로 prefill 한다(`require-named-session` 의 getMe 패턴 미러, 토큰 Bearer 헤더만). 게스트/익명/세션 없음/`GET /me` 오류는 빈 값 폴백(공개 랜딩이라 가드·리다이렉트 없음 — 게스트 진입 보존). 사용자는 prefill 값을 그대로 편집 가능하다(이 모임만의 다른 닉네임 가능 — prefill 은 기본값). 모바일 딥링크 자동 열기(v0.4.0)·기존 수락 흐름(익명 로그인 → 닉네임 → submitAccept → `/moims/:id/chat`)은 Client 폼이 그대로 보존. 검증(2026-06-22): web typecheck/lint/`nx run web:build` 0 + 브라우저 실증 — 로그인 회원(앨리스) 초대 페이지 input "앨리스" prefill 확인(스크린샷) + 게스트(세션 없음, 격리 컨텍스트) 빈 input 확인. status `completed` 유지(UX 개선 — 기존 AC 무파손, 기존 수락 흐름 보존).
