// 네이티브 세션 토큰 동기화 브리지 — supabase 도메인 핸들러(SPEC-MOBILE-002 R-T3/R-T7/R-R1, MOBILE-003 F1/F1').
//
// bridge-transport 코어(전송/인증) 위에 얹혀, 네이티브↔웹 세션 토큰의 검증/갱신/announce 를 담당한다.
// @supabase/supabase-js 에 의존하는 유일한 브리지 모듈 — 이 모듈을 소비하는 진입점(NativeBridgeProvider,
// ShellSessionAnnouncer)만 supabase chunk 를 로드하고, 그들은 셸 모드 가드 통과 후 dynamic import 한다.
// 순수 시그널(google/invite/nav/cleared)은 bridge-signals.ts 로 분리돼 supabase 를 끌어오지 않는다.
//
// 흐름(R-T3): 네이티브가 postMessage 로 session:restore / resume:revalidate 를 주입하면(코어가 origin+nonce
//   인증 후 디스패치), browser supabase 클라이언트로 setSession({access,refresh}) 을 호출해 검증/갱신한다:
//     - valid/refreshed → 최신 토큰을 session:synced 로 회신(OD-9).
//     - empty/expired(data.session === null) → session:none(웹 가드가 /login 라우팅).
//     - throw/네트워크 예외 → session:none(R-T3 throw 폴백 — 핸드셰이크 미해결 방지).
// 보안(SPEC-MOBILE-002 v0.2.0): origin+nonce 인증은 bridge-transport.installInboundListener 가 소유한다
//   (약화 금지). 토큰 값을 로깅하지 않고 postMessage 로만 회신한다(R-V2).
"use client";
 ...
 *
 * 네이티브가 session:restore / resume:revalidate 를 주입하면 bridge-transport 코어가 origin+nonce 인증 후
 * 디스패치하고, 여기서는 restore/revalidate 만 필터링해 setSession 검증/갱신 후 synced/none 을 회신한다.
 * 핸들러는 mount 즉시 등록되므로, 네이티브의 bounded 재시도(R-T7)가 onLoadEnd↔핸들러 등록 race 를 흡수해
 * 메시지 미유실을 보장한다.
 *
 * @returns 리스너 해제 함수(컴포넌트 unmount cleanup). 브리지 없으면 no-op cleanup.
 */
export function installNativeTokenBridge(): () => void {
  return installInboundListener((message, bridge) => {
