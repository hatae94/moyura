...
// 진입 시 /home 폴백(router.replace)을 실행해야 헤더 back 이 실제로 동작한다. bridge-signals.installNavBackListener
...
// 로드 최적화: bridge-signals 는 supabase 무의존이라 정적 import 해도 supabase 오염은 없으나, 셸에서만 필요하므로
//   WebView 가드 통과 후에만 dynamic import 해 데스크톱 (main) 번들에서 제외한다(셸 전용 로드). NavBackNavigator
//   는 import type(런타임 미포함)으로만 참조한다.
"use client";

import { useRouter } from "next/navigation";
import { useEffect } from "react";

import type { NavBackNavigator } from "@/lib/native-bridge/bridge-signals";

// bridge-signals 를 런타임 static import 하지 않으므로(셸 전용 로드), 가드 분기용 Window 타입만 로컬 재선언한다.
declare global {
  interface Window {
...
export function NavBackListener(): null {
  ...
  useEffect(() => {
    // R-T4 가드(앞당김): 일반 브라우저/SSR 이면 bridge-signals 를 import 하기 전에 즉시 bail out.
    ...
    //   판정(window.history.length)은 bridge-signals.installNavBackListener 가 소유한다 — 여기선 router 만 위임한다.
    const navigate: NavBackNavigator = {
      back: () => router.back(),
      replace: (path) => router.replace(path),
    };

    void import("@/lib/native-bridge/bridge-signals")
