...
//   기존 닉네임을 prop 으로 내려 input 초기값으로 prefill 한다(게스트/익명/세션 없음은 빈 값). 사용자는 그대로
//   편집 가능하다(prefill 은 기본값일 뿐 — 이 모임만의 다른 닉네임으로 바꿀 수 있다).
//
// 리다이렉트 대상 /moims/[id]/chat 화면은 SPEC-CHAT-001 범위다 — 경로는 문자열이라 web build는 깨지지 않는다.
"use client";
...
export function InviteAcceptForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // 가입 성공 → 대상 모임의 채팅 화면으로 이동(SPEC-CHAT-001, cross-SPEC 의존).
