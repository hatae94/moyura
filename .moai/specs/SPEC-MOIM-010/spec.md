...
- 2026-06-22 (v0.3.0): device-gated 검증 완료 → status in-progress → completed. 장소 투표 마감 → 모임 헤더 location 확정 갱신(동점/무표 notice + 장소 불변) 검증 완료. 근거: (1) Maestro iOS 시뮬레이터(iPhone 16) hands-free in-WebView 검증(2026-06-22) — 네이티브 셸 부팅 + 로그인(하태용) + 모임 상세 네이티브 push + 전 poll WebView 렌더 + finalize 결과 모임 헤더(장소) 반영 + invite-accept + CHAT-001 채팅 렌더. (2) 데스크톱 멀티탭 워크스루(chrome-devtools, alice/bob 격리 세션) — 장소 투표 생성·투표·생성자 마감 → 헤더 location 확정 갱신·동점 skip. (3) 백엔드 LIVE 종단 E2E(poll-place-finalize.live.mts 13/13, 실 Supabase) + backend jest 308 그린. 참고: Maestro poll-option 직접 탭은 a11y resolution + Next dev badge overlay로 불안정(도구 한계, 앱 결함 아님) — 투표/finalize 자체는 데스크톱 멀티탭 + live.mts로 실증.
- 2026-06-22 (v0.2.0): 구현 완료 + 백엔드/LIVE 종단 검증(poll-place-finalize.live.mts 13/13 PASS) → status in-progress(device-gated). **구현 요약**: (1) **마이그레이션 없음** — `Poll.kind`는 이미 존재하는 string 컬럼이고 `"place"`는 새 허용 VALUE 추가일 뿐이라 DDL/스키마 변경이 없다(`Moim.location`도 MOIM-004 기존 컬럼 재사용). `prisma migrate status` 변경 없이 clean(13 마이그레이션 → 본 SPEC 후에도 13 — 마이그레이션 추가 없음). (2) **백엔드 NestJS**: `parseKind`가 `"place"` 허용으로 확장(미지 값 400 보존). `PollController.create` — kind="place"이면 `normalizeOptions`(일반과 동일 경로, ISO 파싱 안 함), optionDates 빈 배열. `PollService.closePoll` finalize 분기 확장: `kind="date"` → `MoimService.setStartsAt(winner.optionDate)`[MOIM-008, 무변경]; **`kind="place"` → 단일 최다 득표 판정 → 단일 승자면 `MoimService.setLocation(moimId, winner.label)` 호출 + finalizedLocation 설정 / 동점 → 'tie' / 무표 → 'no_votes'**[신규]; 일반 → finalize 없음. `MoimService.setLocation(moimId, location)` 신규 메서드(location 쓰기 단일 출처 — setStartsAt 미러). `PollResponseDto`에 `finalizedLocation: string | null` 추가. `aggregatePolls`에 `finalizedLocation: null` 추가(vote/list 응답). `CreatePollDto.kind @ApiProperty` enum에 "place" 추가. `PollWithResults` 인터페이스 finalizedLocation 추가. api-client `schema.d.ts` 재생성(kind "place" + finalizedLocation). (3) **웹**: `PollWithResults`(kind union `"general"|"date"|"place"` 확장, close 결과 finalizedLocation 추가). `createPollAction` — kind="place"이면 옵션 텍스트 그대로 전달(ISO 변환 없음). `closePollAction` — finalizedLocation 결과 전달. `CreatePollForm` 이진 "일정 투표" 토글 → **3-way 선택**(일반/날짜/장소, `name="kind"`, "날짜" = datetime-local 옵션, "장소"/"일반" = 텍스트 옵션). `PollCard` — kind="place"이면 옵션 label(장소명) 텍스트 렌더(날짜 포맷 없음), 열린 장소 투표 확정 힌트("마감하면 최다 득표 장소가 모임 장소로 확정돼요"), 동점 notice. `revalidatePath`로 모임 헤더 location 갱신(page.tsx 이미 location 렌더 — 추가 코드 없음). (4) **LIVE 종단 증명 스크립트** `apps/backend/test/poll-place-finalize.live.mts` 신규(poll-finalize.live.mts 미러). (5) **모바일 무변경** — 장소 투표 UI는 in-WebView, 네이티브 코드 없음. **자동 게이트**: web tsc/lint/nx build 0, backend jest 308/308(장소 투표 신규 + location finalize 단일 승자/동점/무표/일반/날짜 회귀 + 비생성자 403 + 미지 kind 400 포함), mobile vitest 215/215(회귀 0), prisma migrate status clean(마이그레이션 없음 — 13 마이그레이션 그대로). **LIVE 종단 E2E(2026-06-22 — poll-place-finalize.live.mts, 실 Supabase 스택)**: 13/13 PASS — 단일 승자 → Moim.location 설정 + finalizedLocation 확인, 동점 → skip + location 불변, 무표 → skip, 일반 투표 close → finalizedLocation null, 비생성자 close → 403, 미지 kind → 400. **미완료 device-gated**: 웹 UI 브라우저 워크스루(web 세션 만료) + 모바일 iOS WebView 장소 투표 생성/투표/마감 → 모임 헤더 location 확정 갱신 검증(SPEC-MOIM-003..009와 배치). 자동 게이트 + LIVE 단독으로 completed 전환 불가(프로젝트 메모리 규칙: mobile WebView SPEC device-gated).
