...
- **일정 투표 자동 확정 — 날짜 투표 마감 시 단일 최다 득표 날짜 → Moim.startsAt** (SPEC-MOIM-008 — **in-progress** — backend jest 301/301(날짜 투표 생성 + finalize 신규 + 일반/마감 회귀 포함), tsc 0(all), web lint/build 0, mobile vitest 215/215(회귀 0), prisma migrate clean(`20260621000000_add_poll_kind_option_date` — Poll.kind TEXT NOT NULL DEFAULT 'general' + PollOption.option_date nullable 추가, PollVote PK 무변경, row 손실 0, enum 회피) / 백엔드 라이브 E2E(poll-finalize.live.mts) 15/15 PASS 2026-06-21 / 브라우저 웹 UI 워크스루 + 모바일 iOS WebView device-gated → in-progress 유지): SPEC-MOIM-007(투표 마감)의 직속 후속. "언제 모일까" 날짜 투표를 생성자가 마감하면 단일 최다 득표 날짜가 Moim.startsAt 으로 자동 확정된다.
...
- **투표 마감(deadline + 수동 마감) — 마감 후 투표 차단** (SPEC-MOIM-007 — **in-progress** — backend jest 290/290(마감 신규 + 열린 poll 회귀 포함), tsc 0(all), web lint/build 0, mobile vitest 215/215(회귀 0), prisma migrate clean(`20260620200000_add_poll_closes_at` — Poll.closesAt nullable 추가, PollVote PK 무변경, row 손실 0) / 데스크톱 브라우저 + API 라이브 검증 2026-06-20 PASS / 모바일 WebView poll 마감 인터랙션 device-gated → in-progress 유지): SPEC-MOIM-006(다중 선택 투표)의 직속 후속. 모임 투표에 마감 시각(deadline) + 수동 마감(manual close) + 마감 후 투표 차단(409) 추가.
...
- **투표 다중 선택(multi-select) — 가능한 항목 모두 선택** (SPEC-MOIM-006 — **in-progress** — backend jest 269/269(+11, 단일 회귀 포함), tsc 0(all), web lint/build 0, mobile vitest 215/215(회귀 0), prisma migrate clean(`add_poll_multi_select` — 기존 단일 표 row 손실 0) / 데스크톱 브라우저 + API 라이브 검증 2026-06-20 PASS / 모바일 WebView poll 인터랙션 device-gated → in-progress 유지): SPEC-MOIM-005(단일 선택 투표)의 직속 후속. 모임에서 가장 자주 쓰이는 가용성 투표("가능한 날짜 모두 선택")를 지원하는 **poll별 opt-in 다중 선택** 추가.
...
- **모임 투표(poll) — 생성·단일 투표·결과 집계** (SPEC-MOIM-005 — **in-progress** — backend jest 258/258(poll 36), branch 85.14%, tsc 0(all), web lint/build 0, mobile vitest 215/215(회귀 0), prisma migrate clean / 데스크톱 브라우저 라이브 검증 2026-06-19 PASS / 모바일 WebView Server Action+revalidatePath device-gated → in-progress 유지): 모임을 "일정·장소를 가진 이벤트"에서 "구성원이 의사결정할 수 있는 이벤트"로 진전. 이벤트 트라이어드(일정·장소·**투표**)의 마지막 조각 — 제품 태그라인("일정, 장소, 투표를 한곳에서") 완성.
