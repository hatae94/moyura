...
│  │  │  ├─ poll/       # 투표 도메인 모듈 (SPEC-MOIM-005) — PollModule/PollService/PollController + dto(create-poll/vote/poll-response) + *.spec.ts. @Controller('moims/:id/polls'): POST /(생성, 멤버 전용, question+옵션≥2, 빈 question/옵션<2→400, 비멤버→403), POST /:pollId/vote(단일 투표 upsert, 재투표=교체, 잘못된 optionId→400), GET /(voteCount+myVote+비멤버→403). PollService가 MoimService.assertMember 재사용(단일 출처 불변). Poll/PollOption/PollVote 3 테이블 additive 추가(moim 무변경). PollVote(pollId,userId) 복합 PK = 멤버당 한 투표 불변식. jest poll 36 케이스.
...
│  │  ├─ prisma/        # schema.prisma (Profile(name? — SPEC-MOBILE-004) + Moim(startsAt?/location? — SPEC-MOIM-004, polls Poll[] 역참조 — SPEC-MOIM-005) + MoimMember + MoimInvite + ChatMessage + DeviceToken + Poll/PollOption/PollVote(SPEC-MOIM-005) 모델) + migrations/20260602095934_init_profile + 20260613155202_add_moim + 20260613171209_add_moim_invite + 20260613175232_add_chat + 20260614_add_device_token + 20260615000000_add_profile_name(SPEC-MOBILE-004) + 20260619000000_add_moim_event_fields(SPEC-MOIM-004) + 20260619100000_add_poll(SPEC-MOIM-005 — Poll/PollOption/PollVote 신규 3 테이블)
...
│     │  │        ├─ polls-section.tsx # Client 컴포넌트(SPEC-MOIM-005) — 투표 목록(질문+옵션 득표 막대/퍼센트+내 표 강조)+단일 선택 투표 컨트롤+생성 폼(useActionState)+빈 상태. Meetup 오렌지 토큰
│     │  │        └─ poll-actions.ts  # Server Action(SPEC-MOIM-005, "use server") — createPollAction(질문/옵션 검증→세션→createPoll→revalidatePath/오류) + voteAction(optionId→세션→votePoll→revalidatePath)
...
| `@moyura/api-client` | `packages/api-client` | OpenAPI 생성 타입드 API 클라이언트 | `openapi-typescript 7.13.0` 타입(`src/schema.d.ts`, gitignore) + 얇은 fetch 래퍼(`createApiClient`, `getHealth`, optional `getToken`→Bearer, `getMe`, `listMoims`(SPEC-MOIM-003 completed), `createMoim`+`CreateMoimRequest`(SPEC-MOIM-004), poll 타입 별칭 `CreatePollRequest`/`VoteRequest`/`PollResponse`(SPEC-MOIM-005)) | **구현됨** (SPEC-ENV-SETUP-001 + SPEC-AUTH-001 + SPEC-MOIM-003 + SPEC-MOIM-004 + SPEC-MOIM-005) |
