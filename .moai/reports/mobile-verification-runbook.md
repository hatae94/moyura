...
2. **기대**: 로드 즉시 `moyura://invite/{token}` 이 자동 발화되어 iOS 시스템 "열기?" 확인이 뜬다(v0.4.0 — 버튼 탭 불필요). "앱에서 열기" 버튼도 수동 재시도용으로 노출(모바일 한정).
3. 시스템 확인 "열기"(또는 "앱에서 열기" 버튼) → **기대**: 앱이 `app/invite/[token]` 라우트로 열린다.
4. WebView 가 수락 페이지 로드 + 닉네임 → 수락 흐름 확인.

- [ ] 모바일 Safari 로드 시 scheme 자동 발화(iOS "열기?" 확인) + "앱에서 열기" 버튼 노출(수동 재시도)
- [ ] 자동 발화(또는 "앱에서 열기") → "열기" → 앱 열림 → WebView 수락 페이지

#### 앱 미설치 시 폴백 (optional — 시뮬레이터 환경에서 검증 어려울 수 있음)

1. 시뮬레이터 Safari 에서 수락 페이지 열기.
2. 로드 시 자동 발화(또는 "앱에서 열기" 클릭)된 scheme 이 no-op 이어도 닉네임 폼이 그대로 유지되는지 확인.
3. **기대**: scheme 자동 발화가 no-op(미설치)이면 웹 폼에 머무른다(앱 미설치 시 iOS 시스템 확인이 1회 뜰 수 있으나 폼 보존 — 확인창 없는 무탭은 Universal Links 필요, 향후).

- [ ] 앱 미설치/scheme no-op 시 웹 닉네임 폼 폴백 유지(자동 발화 실패해도 폼 보존)
