...
- **네이티브 Google 로그인 + provider-agnostic 이름 온보딩** (SPEC-MOBILE-004 — 자동 게이트 통과 — backend jest 214/214 85.36% branch, mobile vitest 187/187, tsc 0 errors, web build OK, expo export OK, evaluator-active Overall PASS Func 75/Sec 75/Craft 75/Consistency 90 / AC-1~3·5·6a·6b device-gated EAS dev build + 실 Google 계정 필요 → in-progress): 모바일 앱스토어 제출 준비를 위해 WebView 내 Google 버튼을 네이티브 SDK로 인터셉트하고, 신규/기존 사용자 모두 이름을 수집하는 provider 비종속 온보딩을 구현.
  ...
  - **mobile 네이티브 Google Sign-In 코어**: `apps/mobile/lib/auth/google-signin-core.ts` + `signin-id-token-core.ts`(순수 vitest 코어, RN import 없음, 토큰/자격증명 미로깅). `google-signin.ts` + `supabase-mobile.ts` 얇은 SDK 래퍼(Original Free API). `useAuthBridge.ts` oauth-intercept → 네이티브 경로(google-signin → signInWithIdToken → saveTokens → injectRestore session:restore v1) 전환. `@MX:ANCHOR` 보존, bridge-protocol v1 무변경.
  - **신규 의존성**: `@react-native-google-signin/google-signin@16.1.2`(mobile), `@supabase/supabase-js@2.106.2`(mobile). `app.json` config plugin + iosUrlScheme 플레이스홀더.
  - **테스트**: backend jest 214/214(profile 38 포함 — PATCH /me 유효성·업데이트·400·403 검증), mobile vitest 187/187(google-signin-core 16 + signin-id-token-core 20 신규). tsc 0 errors(backend/web/api-client). web build OK, expo export OK.
