...
- 2026-06-22 (v0.3.0): device-gated 검증 완료 → status in-progress → completed. 모바일 WebView 마감 poll 흐름("마감됨" 배지 + 투표 컨트롤 비활성 + 결과 유지 + 마감 후 409 차단 + 비생성자/마감 "마감하기" 미노출) 렌더 검증 완료. 근거: (1) Maestro iOS 시뮬레이터(iPhone 16) hands-free in-WebView 검증(2026-06-22) — 네이티브 셸 부팅 + 로그인(하태용) + 모임 상세 네이티브 push + 전 poll WebView 렌더(마감 배지 포함) + invite-accept + CHAT-001 채팅 렌더. (2) 데스크톱 멀티탭 워크스루(chrome-devtools, alice/bob 격리 세션) — 미래 마감 생성·열린 poll 투표·생성자 수동 마감·마감 후 409 차단·재-close 멱등. (3) 라이브 통합(poll-*.live.mts) + backend jest 308 그린. 참고: Maestro poll-option 직접 탭은 a11y resolution + Next dev badge overlay로 불안정(도구 한계, 앱 결함 아님) — 투표/마감 자체는 데스크톱 멀티탭 + live.mts로 실증.
