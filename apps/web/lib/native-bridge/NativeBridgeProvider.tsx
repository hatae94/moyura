...
//
// 번들 최적화(First Load JS): bridge-client 는 @supabase/supabase-js(GoTrue+Realtime+Postgrest, ~238KB)
//   를 transitively 끌어온다. 그러나 supabase 는 네이티브 WebView(window.ReactNativeWebView)에서만 실제로
//   호출되므로, 일반 브라우저에서는 static import 를 제거하고 WebView 가드 통과 후에만 dynamic import("./bridge-client")
//   한다. 이로써 번들러가 bridge-client+supabase 를 별도 async chunk 로 분리하고, 순수 웹 라우트(/home 등)의
...
// bridge-client 를 더 이상 static import 하지 않으므로(supabase chunk 분리 목적), 가드 분기에 필요한
// Window.ReactNativeWebView 타입만 로컬로 재선언한다. import type 도 가능하나, runtime 코드를 전혀
// 끌어오지 않도록 최소 declare global 을 택한다(번들 영향 0). bridge-client.ts 의 선언과 호환된다.
declare global {
  interface Window {
 ...
 * 아무것도 하지 않는다(no-op, R-T4) — bridge-client(=supabase) 를 import 조차 하지 않는다. 렌더 출력은
 * 없다(null) — 부수효과(등록/해제)만 담당한다.
 */
export function NativeBridgeProvider(): null {
  useEffect(() => {
    // R-T4 가드(앞당김): 일반 브라우저/SSR 이면 bridge-client 를 import 하기 전에 즉시 bail out 한다.
    // → 번들러가 bridge-client+supabase 를 async chunk 로 분리하고, 브라우저는 그 chunk 를 받지 않는다.
