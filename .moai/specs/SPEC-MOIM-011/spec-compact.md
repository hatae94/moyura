...
- 웹→앱 = 모바일 로드 시 scheme **자동 발화**(앱 셸 제외, v0.4.0) + "앱에서 열기" 버튼(수동 재시도, 모바일 한정). 미설치 → scheme no-op + 웹 닉네임 폼 폴백. 데스크톱·앱 셸 미노출.
...
| REQ-MOIM11-005 | 웹 수락 자동 열기 + "앱에서 열기" 버튼 — 모바일 한정(데스크톱·앱 셸 미노출), 모바일 로드 시 `window.location=moyura://invite/{token}` 1회 자동 발화(v0.4.0) + 버튼 수동 재시도, 기존 닉네임 폼/익명/submitAccept/리다이렉트 보존, scheme 실패 시 웹 폴백 | AC-5 |
