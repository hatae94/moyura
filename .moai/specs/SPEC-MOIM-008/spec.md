...
- 2026-06-22 (v0.3.0): device-gated 검증 완료 → status in-progress → completed. 날짜 투표 마감 → 모임 헤더 startsAt 확정 갱신(동점/무표 notice + 일정 불변) 검증 완료. 근거: (1) Maestro iOS 시뮬레이터(iPhone 16) hands-free in-WebView 검증(2026-06-22) — 네이티브 셸 부팅 + 로그인(하태용) + 모임 상세 네이티브 push + 전 poll WebView 렌더 + finalize 결과 모임 헤더(일정) 반영 + invite-accept + CHAT-001 채팅 렌더. (2) 데스크톱 멀티탭 워크스루(chrome-devtools, alice/bob 격리 세션) — 날짜 투표 생성·투표·생성자 마감 → 헤더 startsAt 확정 갱신·동점 skip. (3) 백엔드 라이브 E2E(poll-finalize.live.mts 15/15, 실 Supabase) + backend jest 308 그린. 참고: Maestro poll-option 직접 탭은 a11y resolution + Next dev badge overlay로 불안정(도구 한계, 앱 결함 아님) — 투표/finalize 자체는 데스크톱 멀티탭 + live.mts로 실증.
- 2026-06-21 (v0.2.0): 구현 완료 + 백엔드 라이브 E2E 검증(poll-finalize.live.mts 15/15 PASS) → status in-progress(device-gated). **구현 요약**: (1) **백엔드 스키마(비파괴 마이그레이션)** — `Poll.kind String @default("general") @map("kind")` + `PollOption.optionDate DateTime? @map("option_date")` 2컬럼 additive 추가. 마이그레이션 `20260621000000_add_poll_kind_option_date`(`ALTER TABLE poll ADD COLUMN kind TEXT NOT NULL DEFAULT 'general'; ALTER TABLE poll_option ADD COLUMN option_date TIMESTAMP(3);` — row 손실 0, PollVote 복합 PK `(pollId,optionId,userId)` 무변경, enum 회피). (2) **날짜 투표 생성** — `POST /moims/:id/polls` 가 optional `kind` 수용(`parseKind` 헬퍼: 생략→"general", 미지값→400). `kind="date"` 면 `options[]` 를 ISO 날짜로 파싱(`parseOptionDates` 헬퍼: 무효 ISO → 400 — closesAt 정책 미러) → `optionDate=파싱시각`, `label=ISO`(정규). `kind="general"` 은 MOIM-005/006/007 그대로(optionDate=null). (3) **finalize(자동 확정)** — `POST .../close`(MOIM-007 생성자 전용) 가 `closesAt=now` 마감 후 `poll.kind==="date"` 이면 옵션 voteCount 집계 → 단일 최다 득표 판정 → 단일 승자면 `MoimService.setStartsAt(moimId, winner.optionDate)` 호출(startsAt 쓰기 단일 출처 신규 메서드) + `finalizedStartsAt` 설정; 동점(`"tie"`) / 무표(`"no_votes"`) 는 finalize 스킵(startsAt 불변); `kind="general"` 은 finalize 없음(MOIM-007 그대로). 비생성자/비멤버 → 403(finalize 미실행). (4) **close 응답 + 읽기 모델** — `PollResponseDto` 에 `kind`/`finalizedStartsAt`(ISO|null)/`finalizeSkippedReason`("tie"|"no_votes"|null) + `PollOptionResponseDto` 에 `optionDate`(ISO|null) 추가(순수 추가, 기존 필드 보존). vote/list 응답에서는 finalize 2필드 항상 null. (5) **api-client** — `schema.d.ts` 재생성, `CreatePollRequest`(kind 추가)/`PollResponse`(kind/optionDate/finalize 반영), 별칭 유지. (6) **웹** — `CreatePollForm` "일정 투표" 토글(kind="date"이면 datetime-local 옵션 전환), `PollCard` kind="date" 옵션 포맷 날짜 렌더(raw ISO 금지) + 확정 힌트 + 동점/무표 notice, `closePollAction` finalize 결과 전달, `revalidatePath` 로 모임 헤더 startsAt 갱신(formatMoimSchedule 반영). `page.tsx` 무변경(startsAt은 revalidatePath 재렌더에 반영). (7) **모바일 무변경** — 날짜 투표 UI 는 `/home/{id]` 내 in-WebView 렌더, 신규 네이티브 코드 없음. **자동 게이트**: backend jest 301/301(날짜 생성 + 무효 날짜 400 + finalize 단일 승자→startsAt + 동점→skip + 무표→skip + 일반 투표 finalize 안 함 + 덮어쓰기 + 비생성자 403 + MOIM-005/006/007 회귀), tsc 0(backend/web/api-client), web lint 0, nx run web:build 0, mobile vitest 215/215(회귀 0), prisma migrate status clean. **백엔드 라이브 E2E(2026-06-21 — poll-finalize.live.mts, 실 Supabase 스택)**: 15/15 PASS — 단일 승자→Moim.startsAt 설정+finalizedStartsAt 확인, 동점(2 voters)→skip+startsAt 불변, 무표→skip, 일반 투표 close→finalize 없음, 비생성자 close→403, 무효 날짜 옵션→400. **미완료 device-gated**: 브라우저 웹 UI 워크스루(moyura-verify 세션 access_token 만료 — 재로그인 필요) + 모바일 iOS WebView 검증. 자동 게이트 단독으로 completed 전환 불가(프로젝트 메모리 규칙: mobile-spec-device-gated). mobile vitest 215/215(회귀 0 — 모바일 무변경).
